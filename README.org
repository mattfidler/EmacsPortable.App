...
then the EmacsPortable.App will write the Proxy settings to Putty, so
that plink connections work regardless of the proxy that you are using.
* Icons
...
** TODO Change PuTTY Proxy settings when proxy settings are detected.
