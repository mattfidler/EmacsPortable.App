...
(defun w32shell-emacsw32-gnuwin32-bindir()
  ;;(lwarn '(w32shell-emacsw32-gnuwin32-bindir) :warning
  ;;"exec-directory=%s" exec-directory)
  (if (fboundp 'usb-site-lisp-dir)
      (progn
        (directory-file-name expand-file-name (concat usb-site-lisp-dir "../App/gw32/bin")))
...
Returns non-nil if success."
  ...
  (let (bin shell)
    ...
    (when (or bin shell)
      ...
      (when shell
        ;;(lwarn '(w32shell) :warning "bin=%s, shell-name=%s" bin
        ;;shellname)
        (when bin
...
(defcustom w32shell-msys-bin (if (not (boundp 'usb-drive-letter))
                                 ""
                               (if (file-exists-p
