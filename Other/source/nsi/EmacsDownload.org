...
#+END_SRC
*** Setup Functions
**** Setup Emacs Versions for Download
#+BEGIN_SRC nsis
  Function SetupVersions
    ...
    loop:
      ...
      Pop $R1
      EnumINI::KeyExist "${mirror}" $R1 "win"
      Pop $R2
      StrCmp $R2 "1" 0 next
      ${NSD_CB_AddString} $hCtl_download_DownloadVersion $R1
    next:
      IntOp $R0 $R0 - 1
      Goto loop
    done:
      ReadIniStr $R0 "${mirror}" "emacs" "default.ver"
      ${NSD_CB_SelectString} $hCtl_download_DownloadVersion $R0
      ClearErrors
  FunctionEnd
