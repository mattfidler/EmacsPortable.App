      ...
      ${ElseIf} $EXEFILE == "EmacsBuild.exe"
        ...
        done_emacs_build:
          ...
          ${Else}
            ...
            FileWrite $4 "@$EXEDIR\App\gw32\bin\make.exe all EMACS=$\"$EXEDIR\App\$2\bin\emacs.exe$\" MAKENSIS=$\"$EXEDIR\App\NSIS\makensis.exe$\"$\r$\n"
            FileWrite $4 "@pause$\r$\n"
            FileClose $4
            ExecDos::exec "cmd /c $\"$TEMP\ep\build.cmd$\""
            Delete "$TEMP\ep\build.cmd"
          ${EndIf}
          ${If} $option_build == "1"
