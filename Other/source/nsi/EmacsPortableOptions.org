...
#+BEGIN_SRC nsis
  ...
  Function fnc_behavior_SetVersions
    ReadIniStr $4 "$EXEDIR\Data\ini\EmacsPortableApp.ini" "EmacsPortableApp" "Version"
    ClearErrors
    StrCpy $4 ""
    ...
    done_emacs:
      ...
      StrCmp $4 "" 0 +2
      StrCpy $4 $2
      ${If} ${FileExists} "$EXEDIR\App\emacs-$4\etc"
        ${NSD_CB_SelectString} $hCtl_behavior_DefaultVersion $4
      ${EndIf}
      ${If} ${FileExists} "$EXEDIR\App\emacs-$4\MacOS"
        ${NSD_CB_SelectString} $hCtl_behavior_MacVersion $4
      ${EndIf}
  FunctionEnd
