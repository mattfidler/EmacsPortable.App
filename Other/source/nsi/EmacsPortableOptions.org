...
#+BEGIN_SRC nsis
  Function SetAssoc
    ...
    ReadIniStr $R0 "$EXEDIATA\Data\ini\EmacsPortableApp.ini" "Assoc" 
    WriteIniStr "$EXEDATA\Data\ini\assoc-io.ini" "Field ${as_as}" "State" "$R0"
...
#+BEGIN_SRC nsis
  Function ValidateAssoc
    ...
    as:
      Call RemoveAssoc
      ReadIniStr $R0 "$EXEDATA\Data\ini\assoc-io.ini" "Field ${as_as}" "State"
      WriteIniStr "$EXEDIATA\Data\ini\EmacsPortableApp.ini" "Assoc" "$R0"
      Abort
    del:
      Call ClearSteal
      Call ClearAssoc
