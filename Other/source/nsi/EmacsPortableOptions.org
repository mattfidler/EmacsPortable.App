...
#+PROPERTY: header-args :tangle EmacsPortableOptions.nsi
* Introduction
...
#+END_SRC
** Fix for invalid verbosity level
See https://stackoverflow.com/questions/47691702/nsis-script-failed-invalid-verbose-level
#+BEGIN_SRC nsis
...
#+BEGIN_SRC nsis
  !macro install7z
    ...
    ${EndIf}
  !macroend
  !define install7z `!insertmacro install7z`
#+END_SRC
...
#+BEGIN_SRC nsis
  !macro installNsis
    ...
    ${EndIf}
  !macroend
  !define installNsis `!insertmacro installNsis`
#+END_SRC
...
#+BEGIN_SRC nsis
  !macro installAhk
    ...
    ${EndIf}
  !macroend
  !define installAhk `!insertmacro installAhk`
  
#+END_SRC


** CopyFiles if Newer macro
#+BEGIN_SRC nsis
  ;%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  ; CopyFilesIfNewer based on MoveFolder
  ;
  ; Author: mlf176f2 changes to MoveFolder to CopyFolder
  ...
  !macro CopyFolder source destination mask
    
    !define COPYFOLDER_JUMP ${__LINE__}
    ...
    ${Locate} "$R0" "/L=D" ".CopyFolder_Locate_createDir"
    
    ; Locate files and move (via callback function)
    
    ${Locate} "$R0" "/L=F /M=${mask} /S= /G=1" ".CopyFolder_Locate_moveFile"
    ...
    !undef COPYFOLDER_JUMP
    
  !macroend
  
  ;==================
  ; CopyFolder macro's ${Locate} callback functions
  ;==================
  
  Function .CopyFolder_Locate_createDir
  ...
  Function .CopyFolder_Locate_moveFile
  ...
  Function .CopyFolder_Locate_deleteDir
  ...
  FunctionEnd
  !define CopyFolder `!insertmacro CopyFolder`
#+END_SRC
...
#+BEGIN_SRC nsis
  Function DownloadAHK
    ...
    ${If} $0 != ""
      ...
      ExecWait '"$EXEDIR\App\7z\7zG.exe" x -y "$TEMP\ep\ep\ahk.7z" -o"$EXEDIR\App"'
...
#+BEGIN_SRC nsis
  Function DownloadNSIS
    ...
    ClearErrors
    ${If} $0 != ""
      Inetc::get "$0" "$TEMP\ep\ep\NSIS.7z"
      ExecWait '"$EXEDIR\App\7z\7zG.exe" x -y "$TEMP\ep\ep\NSIS.7z" -o"$EXEDIR\App"'
      Delete "$TEMP\ep\ep\NSIS.7z"
    ${EndIf}
  FunctionEnd
#+END_SRC
* Pages
...
#+BEGIN_SRC nsis
  ...
  Function fnc_startup_options_Create
    ...
    ${NSD_LV_InsertItem} $hCtl_startup_options_Select 6 "Build/Update Launcher (Requires/Downloads NSIS & AHK)" 
    ...
    ${Else}
      ${If} ${FileExists} "$EXEDIR\App\ini\mirrors.ini"
        ...
        loop_emacs:
          StrCmp $1 "" done_emacs
          ;;FIXME
          ${If} ${FileExists} $EXEDIR\App\$1\etc
            StrCpy $2 "1"
          ${EndIf}
          ${If} ${FileExists} $EXEDIR\App\share
            StrCpy $2 "1"
          ${EndIf}
          FindNext $0 $1
          Goto loop_emacs
...
#+BEGIN_SRC nsis
  ...
  Var hCtl_behavior_DebugLauncher
  Var hCtl_behavior_Label2
  Var hCtl_behavior_Label1
  Var hCtl_behavior_Label4
  Var hCtl_behavior_newFrame
  Var hCtl_behavior_MacVersion
  ...
  Function fnc_behavior_SetStartup
    ReadIniStr $4 "$EXEDIR\Data\ini\EmacsPortableApp.ini" "EmacsPortableApp" "Startup"
    ClearErrors
  ...
  Function fnc_behavior_SetVersions
    ClearErrors
    StrCpy $4 ""
    StrCpy $5 ""
    ...
    loop_emacs:
      ...
      StrCpy $1 $1 "" 6
      StrCpy $2 $1
      ${If} ${FileExists} "$EXEDIR\App\emacs-$1\etc"
        ${NSD_CB_AddString} $hCtl_behavior_DefaultVersion $1
        StrCmp $2 $3 0 +2
        StrCpy $4 $2
      ${EndIf}
      ${If} ${FileExists} "$EXEDIR\App\emacs-$1\share"
        ${NSD_CB_AddString} $hCtl_behavior_DefaultVersion $1
        StrCmp $2 $3 0 +2
        StrCpy $4 $2
      ${EndIf}
      ${If} ${FileExists} "$EXEDIR\App\emacs-$1\MacOS"
        ${NSD_CB_AddString} $hCtl_behavior_MacVersion $1
        StrCpy $5 $2
      ${EndIf}
      FindNext $0 $1
      Goto loop_emacs
    done_emacs:
      ...
      ReadIniStr $R0 "$EXEDIR\Data\ini\EmacsPortableApp.ini" "EmacsPortableApp" "Version"
      ${If} ${FileExists} "$EXEDIR\App\emacs-$R0\etc\DOC-X"
        ${NSD_CB_SelectString} $hCtl_behavior_DefaultVersion $R0
      ${ElseIf} ${FileExists} "$EXEDIR\App\emacs-$4\etc\DOC-X"
        ${NSD_CB_SelectString} $hCtl_behavior_DefaultVersion $4
      ${ElseIf} ${FileExists} "$EXEDIR\App\emacs-$R0\share\emacs\$R0\etc\DOC"
        ${NSD_CB_SelectString} $hCtl_behavior_DefaultVersion $R0
      ${ElseIf} ${FileExists} "$EXEDIR\App\emacs-$4\share\emacs\$4\etc\DOC"
        ${NSD_CB_SelectString} $hCtl_behavior_DefaultVersion $4
      ${EndIf}
      ClearErrors
      
      ReadIniStr $R0 "$EXEDIR\Data\ini\EmacsPortableApp.ini" "EmacsPortableApp" "MacVersion"
      ${If} ${FileExists} "$EXEDIR\App\emacs-$R0\MacOS\Emacs"
        ${NSD_CB_SelectString} $hCtl_behavior_MacVersion $R0
      ${ElseIf} ${FileExists} "$EXEDIR\App\emacs-$5\MacOS\Emacs"
        ${NSD_CB_SelectString} $hCtl_behavior_MacVersion $5
      ${EndIf}
      ClearErrors
  FunctionEnd
  ...
  Function fnc_behavior_Create
    ...
    ${NSD_CreateGroupBox} 8u 7u 280u 78u "EmacsPortable.App Launching Options"
    Pop $hCtl_behavior_GroupBox1
    
    ; === Debug (type: Checkbox) ===
    ${NSD_CreateCheckbox} 167u 58u 109u 14u "Debug Emacs with gdb"
    Pop $hCtl_behavior_Debug
    
    ; === Daemon (type: Checkbox) ===
    ${NSD_CreateCheckbox} 12u 46u 88u 14u "Use Pseudo-Dameon"
    Pop $hCtl_behavior_Daemon
    ${NSD_Check} $hCtl_behavior_Daemon
    
    ; === DebugLauncher (type: Checkbox) ===
    ${NSD_CreateCheckbox} 12u 58u 140u 14u "Debug Launcher Startup"
    Pop $hCtl_behavior_DebugLauncher
    
    ; === Label2 (type: Label) ===
    ${NSD_CreateLabel} 128u 19u 58u 13u "Startup Script"
    ...
    ; === Label4 (type: Label) ===
    ${NSD_CreateLabel} 13u 33u 54u 13u "Mac Version"
    Pop $hCtl_behavior_Label4
    
    ; === newFrame (type: Checkbox) ===
    ${NSD_CreateCheckbox} 167u 47u 95u 14u "New frame on file open"
    Pop $hCtl_behavior_newFrame
    
    ; === MacVersion (type: DropList) ===
    ${NSD_CreateDropList} 70u 31u 51u 12u ""
    Pop $hCtl_behavior_MacVersion
    SetCtlColors $hCtl_behavior_MacVersion 0x000000 0xFFFFFF
    ...
    ; === Home_Txt (type: Text) ===
    ${NSD_CreateText} 190u 33u 73u 11u ""
    Pop $hCtl_behavior_Home_Txt
    
    ; === Home_Btn (type: Button) ===
    ${NSD_CreateButton} 264u 33u 20u 11u "..."
    Pop $hCtl_behavior_Home_Btn
    ${NSD_OnClick} $hCtl_behavior_Home_Btn fnc_hCtl_behavior_Home_Click
    
    ; === Label3 (type: Label) ===
    ${NSD_CreateLabel} 128u 33u 58u 13u "Home Directory"
    Pop $hCtl_behavior_Label3
    
    ; === ZipBin (type: Checkbox) ===
    ${NSD_CreateCheckbox} 13u 69u 151u 14u "(Un)Zip Binaries && Run from %TEMP%"
    Pop $hCtl_behavior_ZipBin
    
    ; === Clean (type: Checkbox) ===
    ${NSD_CreateCheckbox} 167u 69u 117u 14u "Clean Residual Files (Stealth)"
    Pop $hCtl_behavior_Clean
    
    ; === GroupBox2 (type: GroupBox) ===
    ${NSD_CreateGroupBox} 8u 95u 280u 26u "Integration Options"
    Pop $hCtl_behavior_GroupBox2
    
    ; === orgProtocol (type: Checkbox) ===
    ${NSD_CreateCheckbox} 226u 106u 58u 14u "org-protocol"
    Pop $hCtl_behavior_orgProtocol
    
    ; === editWith (type: Checkbox) ===
    ${NSD_CreateCheckbox} 12u 106u 89u 14u "Right-Click $\"Edit With$\""
    Pop $hCtl_behavior_editWith
    
    ; === LiberKey (type: Checkbox) ===
    ${NSD_CreateCheckbox} 167u 106u 47u 14u "LiberKey"
    Pop $hCtl_behavior_LiberKey
    
    ; === SendTo (type: Checkbox) ===
    ${NSD_CreateCheckbox} 105u 106u 47u 14u "Send To"
    ...
    ${SetUnCheck} $hCtl_behavior_DebugLauncher "DebugLauncher"
    ...
    ${SetUnCheck} $hCtl_behavior_SendTo "SendTo"
    
    Call fnc_behavior_SetStartup
    Call fnc_behavior_SetVersions
  FunctionEnd
...
#+BEGIN_SRC nsis
  Function fnc_behavior_Validate
    ...
    ${If} $option_behavior == "1"
      ...
      ${WriteCheck} $hCtl_behavior_DebugLauncher "DebugLauncher"
      ...
      ${WriteCheck} $hCtl_behavior_SendTo "SendTo"
      
      ${WriteText} $hCtl_behavior_Home_Txt "Home"
      # Only write if emacs exists.
      
      ${If} ${FileExists} "$EXEDIR\App\emacs-$hCtl_behavior_DefaultVersion\bin\emacsclient.exe"
      ${OrIf} ${FileExists} "$EXEDIR\App\emacs-$hCtl_behavior_DefaultVersion\bare-bin.7z"
        ${WriteCB} $hCtl_behavior_DefaultVersion "Version"
      ${EndIf}
      ${If} ${FileExists} "$EXEDIR\App\emacs-$hCtl_behavior_DefaultVersion\MacOS\Emacs"
        ${WriteCB} $hCtl_behavior_MacVersion "MacVersion"
      ${EndIf}
...
#+BEGIN_SRC nsis
  Function fnc_proxy_Validate
    ${If} $option_proxy == "1"
      ...
      Pop $R0
      Pop $R0
      WriteIniStr "$EXEDIR\Data\ini\proxy-$PROXY_IDE.ini" "$PROXY_NAME" "ID" "$R0"
    ${EndIf}
  FunctionEnd
        ...
        ${If} ${FileExists} "$EPEXE\App\ini\mirrors.ini"
          ...
          ${If} $option_download == "1"
            ...
            ${IfNot} ${FileExists} "$EPEXE\App\hunspell\bin\hunspell.exe"
              ...
              ${If} $0 != ""
                SetOutPath $TEMP\ep
                Inetc::get "$0" "$TEMP\ep\hunspell.7z"
                SetOutPath "$EXEDIR\App"
                ExecWait '"$EXEDIR\App\7z\7zG.exe" e -y "$TEMP\ep\hunspell.7z" -o"$EXEDIR\App"'
            ...
            ${If} $download_font != "(Select)"
              ...
              ${If} $R0 != ""
                ...
                ${ElseIf} $R1 == "tgz"
                  DetailPrint "tgz"
                  SetOutPath "$TEMP\ep"
                  inetc::get $R0 "$TEMP\ep\font.tgz"
                  ExecWait '"$EXEDIR\App\7z\7zG.exe" e -y "$TEMP\ep\font.tgz" -o$TEMP\ep'
                  Delete "$TEMP\ep\font.tgz"
                  ${If} ${FileExists} "$TEMP\ep\font.tar"
                    ExecWait '"$EXEDIR\App\7z\7zG.exe" e -y "$TEMP\ep\font.tar" -o$EXEDIR\App\fonts'
                    Delete "$TEMP\ep\font.tar"
                  ${EndIf}
                ${Else}
                  DetailPrint "Raw"
                  Push $R0
                ...
                done_fonts:
                  ...
                  ${If} $3 == ""
                    ...
                    Goto  loop_fonts
                  ${EndIf}
                  ; Reset Font Installation.
                  ${If} ${FileExists} "$EXEDIR\App\eps\rm-ep-font.exe"
                    ExecWait "$EXEDIR\App\eps\rm-ep-font.exe"
                  ${ElseIf} ${FileExists} "$TEMP\ep\rm-ep-font.exe"
                    ExecWait "$TEMP\ep\rm-ep-font.exe"
                  ${ElseIf} ${FileExists} "$TEMP\rm-ep-font.exe"
                    ExecWait "$TEMP\rm-ep-font.exe"
                  ${EndIf}
                  ExecWait "$EXEDIR\App\eps\ep-font.exe"
              ${EndIf}
            ${EndIf}
            ${If} ${FileExists} "$EXEDIR\App\emacs-$download_emacs_version\bin\emacsclient.exe"
            ${OrIf} ${FileExists} "$EXEDIR\App\emacs-$download_emacs_version\bare-bin.7z"
              WriteIniStr "$EXEDIR\Data\ini\EmacsPortableApp.ini" "EmacsPortableApp" \
                  "Version" "$download_emacs_version"
            ${EndIf}
            ${If} ${FileExists} "$EXEDIR\App\emacs-$download_emacs_version\MacOS\Emacs"
              WriteIniStr "$EXEDIR\Data\ini\EmacsPortableApp.ini" "EmacsPortableApp" \
                  "MacVersion" "$download_emacs_version"
            ${EndIf}

            FindProcDLL::FindProc "EmacsPortableServer.exe"

            ${If} $R0 == "1"
              WriteIniStr "$TEMP\ep\ep-refresh-server.ini" "one" "one" "one"
            ${EndIf}
          ...
          ${If} $option_request_build == "1"
            ...
            ${installAhk}
            ${installNsis}
            ## First Download the EmacsPortable.App from the github master.
            ...
            loop_epa_build:
              StrCmp $1 "" done_epa_build
              StrCmp $1 "." next_epa_build
              StrCmp $1 ".." next_epa_build
              DetailPrint $1
              StrCpy $R1 $1
            next_epa_build:
            ...
            done_epa_build:
              FindClose $0
              ${CopyFolder}  "$TEMP\ep\ep\$R1\" "$EXEDIR\" "*.*"
              RmDir /r "$TEMP\ep\ep"
      ...
      ${ElseIf} $EXEFILE == "EmacsBuild.exe"
        ...
        loop_emacs_build:
          ...
          ${If} ${FileExists} "$EXEDIR\App\$1\etc"
            StrCpy $7 $1
          ${EndIf}
          ${If} ${FileExists} "$EXEDIR\App\$1\share"
            StrCpy $7 $1
          ${EndIf}
          FindNext $0 $1
        ...
        done_emacs_build:
          ...
          ${Else}
            ;; Unzip binaries.
            ${If} ${FileExists} "$EXEDIR\App\$7\bare-bin.7z"
              ExecWait '"$EXEDIR\App\7z\7zG.exe" x -y "$EXEDIR\App\$7\bare-bin.7z" -o"$EXEDIR\App"'
              ${If} ${FileExists} "$EXEDIR\App\$7\bin\emacs.exe"
                Delete "$EXEDIR\App\$7\bare-bin.7z"
              ${EndIf}
            ${EndIf}
            ${If} ${FileExists} "$EXEDIR\App\$7\ext-bin.7z"
              ExecWait '"$EXEDIR\App\7z\7zG.exe" x -y "$EXEDIR\App\$7\ext-bin.7z" -o"$EXEDIR\App"'
              ${If} ${FileExists} "$EXEDIR\App\$7\bin\emacsclient.exe"
                Delete "$EXEDIR\App\$7\ext-bin.7z"
              ${EndIf}a
            ${EndIf}
