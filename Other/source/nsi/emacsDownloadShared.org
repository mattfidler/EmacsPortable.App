...
#+BEGIN_SRC nsis
  ...
  !macro g32down WHAT
    ...
    inetc::get /CONNECTTIMEOUT 30 $R0 "$EPEXE\App\gw32\bin\${WHAT}.exe"
    ...
    g32_zip:
      ...
      ExecWait '"$EPEXE\App\7z\7zG.exe" x "$TEMP\${WHAT}-bin.zip" -o"$TEMP\ep-gw32"'
      StrCpy $R0 "${WHAT}"
      StrCpy $R1 "bin"
      ...
      RmDir /r "$TEMP\ep-gw32"
      ExecWait '"$EPEXE\App\7z\7zG.exe" x "$TEMP\${WHAT}-dep.zip" -o"$TEMP\ep-gw32"'
...
#+BEGIN_SRC nsis
  ...
  !macro ezwindown WHAT
    ...
    ExecWait '"$EPEXE\App\7z\7zG.exe" x "$TEMP\${WHAT}-bin.zip" -o"$TEMP\ep-ezwin"'
