...
Function RegIE
  ...
  WriteRegStr HKLM "SOFTWARE\Microsoft\Internet Explorer\View Source Editor" "Editor Name" "$\"$\"$EXEDIR\EmacsPortableApp.exe$\" $\"%1$\""
  ...
  WriteRegStr HKCR "htmlfile\shell\edit\command" "" "$\"$EXEDIR\EmacsPortableApp.exe$\" $\"%1$\""
FunctionEnd
...
Function RegOrgProtocol
  ...
  register_protocol:
    ...
    WriteRegStr HKEY_CLASSES_ROOT "org-protocol\shell\open\command" "" "$\"$EXEDIR\EmacsPortableApp.exe$\" $\"%1$\""
...
Function RegEditWith
  ...
  WriteRegStr HKEY_CLASSES_ROOT "*\shell\openwemacs\command" "" "$\"$EXEDIR\EmacsPortableApp.exe$\" $\"%1$\""
FunctionEnd
...
#+BEGIN_SRC nsis
  ...
  !macro _dllcp FILENAME FILETO
    IfFileExists "$emacs\${FILETO}" +4
    IfFileExists "$EXEDIR\App\${FILENAME}" 0 +3
...
#+BEGIN_SRC nsis
  Function CreateZip
    ...
    found_7z:
      ...
      ExecWait  "cmd /c $\"cd $EXEDIR\app & $R0 a -mx9 -tzip emacs-$option_version\bare-bin.zip emacs-$option_version\bin\emacs.exe emacs-$option_version\etc\DOC-X$\""
      ExecWait  "cmd /c $\"cd $EXEDIR\app & $R0 a -mx9 -tzip emacs-$option_version\ext-bin.zip emacs-$option_version\bin\emacsclient.exe emacs-$option_version\bin\cmd*.exe emacs-$option_version\bin\hex*.exe emacs-$option_version\bin\ebr*.exe emacs-$option_version\bin\move*.exe emacs-$option_version\bin\*tags*.exe emacs-$option_version\bin\dd*.exe emacs-$option_version\bin\dd*.exe$\""
...
#+BEGIN_SRC nsis
  Function setupPutty
    ...
    launch_putty:
      Exec "$TEMP\ep\run.exe $R1\PageantPortable.exe $R0"
...
#+BEGIN_SRC nsis
  !macro emacs_setup
    DetailPrint "Extracting run/2.exe"
    SetOutPath "$TEMP\ep"
    IfFileExists $TEMP\ep\run.exe" +4
    File "..\..\..\App\eps\run.exe"
    File "..\..\..\App\eps\run2.exe"
    File "..\..\..\App\eps\EmacsPortableServer.exe"
...
!macro emacsclient_setup
  ...
  setupwin_client:
    StrCmp $cmdLineParams "" +3
    StrCpy $emacs `"$emacs\$option_emacsclient" --server-file "$TEMP\ep\EmacsPortable.App-Server-$option_version\server" $cmdLineParams`
    Goto +2
    StrCpy $emacs `"$emacs\$option_emacsclient" --server-file "$TEMP\ep\EmacsPortable.App-Server-$option_version\server" -e "(new-emacs)"`
  end_client:
!macroend
 ...
 #+BEGIN_SRC nsis
   Section "Main" sec_main
     ...
     loop:
       ...
       StrCpy $cnt 300
       IfFileExists "$TEMP\ep\EmacsPortableServer-$option_version.exe" +3 0
       IfFileExists "$TEMP\ep\EmacsPortableServer.exe" 0 end
       CopyFiles /SILENT "$TEMP\ep\EmacsPortableServer.exe" \
           "$TEMP\ep\EmacsPortableServer-$option_version.exe"
       Exec "$TEMP\ep\EmacsPortableServer-$option_version.exe $option_version"
       Goto end
     start_emacsclient:
       ${emacsclient_setup}
       Exec "$TEMP\ep\run.exe $emacs"
...
#+BEGIN_SRC nsis
  ...
  Section "Uninstall" sec_uninstall ; Checked
    ...
    look_for_running_emacs:
      Sleep 7000 # Wait 7 seconds for emacs process to start.
      StrCpy $nver 0
