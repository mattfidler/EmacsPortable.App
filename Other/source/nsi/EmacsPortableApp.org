  ...
  Function parseParameters
    ...
    loop_find_running:
      ...
      ${If} $R0 == "1"
        ...
        StrCpy $epstart ""
      ${Else}
        FindFirst $3 $4 $EXEDIR\App\emacs-*.*
        ${While} $4 != ""
          ${If} $4 != "."
          ${AndIf} $4 != ".."
          ${AndIf} $4 != "shared"
...
#+BEGIN_SRC nsis
  ...
  !macro _dllcp FILENAME FILETO
    DetailPrint "$EXEDIR\App\${FILENAME} -> $emacs_dir\${FILETO}"
    ${IfNot} ${FileExists} "$emacs_dir\${FILETO}"
      ${If} ${FileExists} "$EXEDIR\App\${FILENAME}"
        CopyFiles /SILENT "$EXEDIR\App\${FILENAME}" "$emacs_dir\${FILETO}"
        ${If} ${FileExists} "$EXEDIR\App\emacs-$option_version\ext-bin.7z"
          SetOutPath "$TEMP\ep"
          ${If} ${FileExists} "$EXEDIR\App\7z\7z.exe"
            nsExec::ExecToLog /OEM "$\"$EXEDIR\App\7z\7z.exe$\" a -mx9 -t7z $EXEDIR\App\emacs-$option_version\ext-bin.7z $R5emacs-$option_version\bin\${FILETO} -ms=off -mmt"
            Push $0
            Exch
            Pop $0
            Pop $0
          ${Else}
            ExecWait "$\"$EXEDIR\App\7z\7zG.exe$\" a -mx9 -t7z $EXEDIR\App\emacs-$option_version\ext-bin.7z $R5emacs-$option_version\bin\${FILETO} -ms=off -mmt"
          ${EndIf}
        ${EndIf}
      ${EndIf}
    ${EndIf}
...
#+BEGIN_SRC nsis
  ...
  extract_zip:
    ${If} ${FileExists} "$EXEDIR\App\emacs-$option_version\bare-bin.7z"
      ${If} ${FileExists} "$EXEDIR\App\7z\7z.exe"
        nsExec::ExecToLog /OEM '"$EXEDIR\App\7z\7z.exe" "$EXEDIR\App\emacs-$option_version\bare-bin.7z" -o"$EXEDIR\App" -mmt'
      ${Else}
        ExecWait '"$EXEDIR\App\7z\7z.exe" "$EXEDIR\App\emacs-$option_version\bare-bin.7z" -o"$EXEDIR\App" -mmt'
      ${EndIf}
      ${If} ${FileExists} "$EXEDIR\App\emacs-$option_version\bin\emacs.exe"
        Delete "$EXEDIR\App\emacs-$option_version\bare-bin.7z"
      ${EndIf}
    ${EndIf}
    ${If} ${FileExists} "$EXEDIR\App\emacs-$option_version\ext-bin.7z"
      ${If} ${FileExists} "$EXEDIR\App\7z\7z.exe"
        nsExec::ExecToLog /OEM '"$EXEDIR\App\7z\7zG.exe" "$EXEDIR\App\emacs-$option_version\ext-bin.7z" -o"$EXEDIR\App" -mmt' 
      ${Else}
        ExecWait '"$EXEDIR\App\7z\7zG.exe" "$EXEDIR\App\emacs-$option_version\ext-bin.7z" -o"$EXEDIR\App" -mmt'
      ${EndIf}
      ${If} ${FileExists} "$EXEDIR\App\emacs-$option_version\bin\emacsclient.exe"
        Delete "$EXEDIR\App\emacs-$option_version\ext-bin.7z"
      ${EndIf}
    ${EndIf}
    Goto end
  zip_binaries:
    IfFileExists "$EXEDIR\App\emacs-$option_version\ext-bin.7z" end
    Pop $R0
    ...
    found_7z:
      ...
      SetOutPath "$EXEDIR\App"
      ExecWait '"$EXEDIR\App\7z\7zG.exe" a -mx9 -t7z emacs-$option_version\bare-bin.7z emacs-$option_version\bin\emacs.exe emacs-$option_version\etc\DOC-X -mmt -ms=on -m0=PPMd'
      ExecWait '"$EXEDIR\App\7z\7zG.exe"  a -mx9 -t7z emacs-$option_version\ext-bin.7z \
  ...
  Section "Main" sec_mainN
    ...
    ${Else}
      ...
      loop:
        ...
        StrCpy $cnt 300
        Exec "$EXEDIR\App\eps\EmacsPortableServer.exe"
