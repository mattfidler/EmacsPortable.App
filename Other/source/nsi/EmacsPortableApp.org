...
#+BEGIN_SRC nsis
  ...
  Function GetCmdOptions
    ...
    StrCpy $emacs_client_exe "emacsclientw.exe"
    ; Parse Parameters
    Push $R0
    ...
    ${IfNot} ${FileExists} "$emacs_dir\${FILETO}"
      ${If} ${FileExists} "$EXEDIR\App\${FILENAME}"
        ...
        ${If} ${FileExists} "$EXEDIR\App\emacs-$option_version\ext-bin.zip"
          SetOutPath "$TEMP\ep"
          ExecWait '"$R4" a -mx9 -tzip $EXEDIR\App\emacs-$option_version\ext-bin.zip $R5emacs-$option_version\bin\${FILETO}'
        ${EndIf}
      ${EndIf}
    ${EndIf}
  !macroend
...
#+BEGIN_SRC nsis
  ...
  zip_binaries:
    ...
    found_7z:
      ...
      SetOutPath "$EXEDIR\App"
      ExecWait '"$R0" a -mx9 -tzip emacs-$option_version\bare-bin.zip emacs-$option_version\bin\emacs.exe emacs-$option_version\etc\DOC-X'
      ExecWait '"$R0"  a -mx9 -tzip emacs-$option_version\ext-bin.zip \
          emacs-$option_version\bin\emacsclient.exe \
          emacs-$option_version\bin\emacsclientw.exe \
...
#+BEGIN_SRC nsis
  Section "Main" sec_mainN
    ...
    loop_proxy:
      ...
      StrCpy $2 $1 "" 6
      IfFileExists "$EXEDIR\App\emacs-$2\bin\emacsclientw.exe" 0 +3
      StrCpy $3 "$EXEDIR\App\emacs-$2\bin\emacsclientw.exe"
      Goto +3
      IfFileExists "$TEMP\ep\emacs-$2\bin\emacsclientw.exe" 0 loop_next
      StrCpy $3 "$TEMP\ep\emacs-$2\bin\emacsclientw.exe"
      ${ExecHide} '"$3" --server-file "$TEMP\ep\EmacsPortable.App-Server-$option_version\server" --eval "(if (fboundp $\'refresh-proxy) (refresh-proxy))"'\
          "$home" $R1
      ${If} $R1 == "0"
        SetOutPath $home
    ...
    start_emacsclient:
      ...
      ${If} $option_help == "1"
        ...
        ${If} $R1 == "0"
          SetOutPath $home
          Exec '"$cmd" $cmd_args > "$TEMP\ep\emacs-help.txt"'
        ${EndIf}
        ClearErrors
        FileOpen $0 "$TEMP\ep\emacs-help.txt" r
        ${DoUntil} ${Errors}
