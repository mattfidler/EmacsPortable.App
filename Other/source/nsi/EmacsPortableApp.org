...
#+END_SRC
** Execute and get process id
:LOGBOOK:
CLOCK: [2012-06-12 Tue 23:23]
:END:
Executes files via the ShellExecuteEx api, uses three
modes: 1. nowait - returns process id, 2. nowait - returns process
handle, and 3. wait mode - returns process exit code. XP-SP1/2003 or
higher is required for mode 1.

http://nsis.sourceforge.net/ShellExecEx
...
#+BEGIN_SRC nsis
  ...
  !macro _dllcp FILENAME FILETO
    IfFileExists "$emacs_dir\${FILETO}" +5
...
#+BEGIN_SRC nsis
  ...
  Function SetupContents
    ...
    ${StrSlash} "$R0" "/"
    ExecDos::exec "$found_emacs\emacs.exe -Q --batch -l $R0 -f build-app-info"
    end:
      ClearErrors
  FunctionEnd
...
#+BEGIN_SRC nsis
  Section "Main" sec_mainN
    ...
    loop_proxy:
      ...
      ${ShellExecEx} $R1 "" "$3" '--server-file "$TEMP\ep\EmacsPortable.App-Server-$option_version\server" --eval "(if (fboundp $\'refresh-proxy) (refresh-proxy))"' "" SH_HIDE "1"
    loop_next:
      FindNext $0 $1
      Goto loop_proxy
    done_proxy:
    ...
    start_emacs_gdb:
      ...
      ${ShellExecEx} $R1 "" "$cmd" "$cmd_args" "" "0" "1"
      Pop $R0
      Goto loop
    ...
    start_emacs_regular:
      ${If} $option_help == "1"
        SetOutPath "$TEMP\ep"
        ${ShellExecEx} $R1 "" "$cmd" '$cmd_args > "$TEMP\ep\emacs-help.txt"' "" SH_HIDE "1"
        ClearErrors
      ...
      ${Else}
        ...
        System::Call 'Kernel32::SetEnvironmentVariableA(t, t) i("TEMP", "$TEMP\ep").r0'
        ##${StrRep} "$emacs" "$emacs" '"' '""'
        ${ShellExecEx} $R1 "" "$cmd" "$cmd_args" "" SH_HIDE "1"
        Pop $R0                  
    ...
    start_emacsclient:
      ...
      ${If} $option_help == "1"
        ...
        ExecDos::exec /TOSTACK "$emacs"
        Pop $R0
        StrCpy $R0 ""
        StrCpy $R1 ""
        Loop_emacsclient_help:
          Pop $1
          StrCmp $1 "ExecDos::End" exit_emacsclient_help
          StrCmp $1 $R1 exit_emacsclient_help
          ${If} $R0 == ""
            StrCpy $R0 "$1"
          ${Else}
            StrCpy $R0 "$1$\n$R0"
          ${EndIf}
          StrCpy $R1 $1
          Goto Loop_emacsclient_help
        exit_emacsclient_help:
          SendMessage $HWNDPARENT ${WM_SETTEXT} 0 "STR:Emacsclient help"
          MessageBox MB_OK|MB_ICONQUESTION "$R0"
